...
The full plan, its priorities, and the per-branch coverage map live in the committed
`docs/test-plan.md`. The first deliverable — the E2E engine and smoke suite — exists on
...
4. **No merge without coverage**: once the suite lands on main, every change merged to main carries
   a test matching its priority tier in `docs/test-plan.md` (P0 smoke + verify-internals at
   minimum).
