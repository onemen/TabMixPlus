    ...
    try {
      ...
      class TabmixArrowscrollbox {
        connectTabmix() {
          ...
          this.innerbox = this.shadowRoot.querySelector("scrollbox").firstChild;
          ...
          this.scrollbox.addEventListener("underflow", event => {
            console.log("underflow NOT on target", event.originalTarget !== this.scrollbox);
            console.log("underflow event.detail", ["vertical", "horizontal", "both"][event.detail]);
          ...
          this.scrollbox.addEventListener("overflow", event => {
            console.log("overflow NOT on target", event.originalTarget !== this.scrollbox);
            console.log("overflow event.detail", ["vertical", "horizontal", "both"][event.detail]);
      ...
      const obj = TabmixArrowscrollbox.prototype;
      const kyes = Object.getOwnPropertyNames(obj);
      for (const key of kyes) {
        if (key !== "constructor") {
          const {get, set, value} = Object.getOwnPropertyDescriptor(obj, key);
          if (get || set) {
            const descriptor = Object.create(null);
            if (get) descriptor.get = get.bind(tabstrip);
            if (set) descriptor.set = set.bind(tabstrip);
            Object.defineProperty(tabstrip, key, descriptor);
          } else if (value) {
            Object.defineProperty(tabstrip, key, {value: value.bind(tabstrip)});
          }
        }
      }
      tabstrip.connectTabmix();
    } catch (ex) {
