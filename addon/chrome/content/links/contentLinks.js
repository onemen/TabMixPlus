...
function TMP_contentLinkClick(aEvent) {
  ...
  if ("ownerDocument" in target && Tabmix.isNewTabUrls(target.ownerDocument.documentURI))
    return;
  ...
  var domain = Tabmix.contentAreaClick.checkDomain(curpage, window.XULBrowserWindow.overLink || target);
 ...
 * @returns			Either the return value of Tabmix.original_contentAreaClick(), or the
...
function TMP_suppressTabsOnFileDownload(event, target, linkNode, suppressTabs) {
   ...
   if (event.button != 2 && typeof(custombuttons) !='undefined'){
      if (Tabmix.contentAreaClick.checkAttr(linkNode.toString(), "custombutton://"))
         return true;
   }
   ...
   if (Tabmix.contentAreaClick.checkAttr(linkNode.toString(), "javascript:"))
      return false;

   if (target.hasAttribute("onclick")) {
      var onclick = target.getAttribute("onclick");
      if (Tabmix.contentAreaClick.checkAttr(onclick, "return install") ||
          Tabmix.contentAreaClick.checkAttr(onclick, "return installTheme") ||
          Tabmix.contentAreaClick.checkAttr(onclick, "return note") || Tabmix.contentAreaClick.checkAttr(onclick, "return log")) // click on link in http://tinderbox.mozilla.org/showbuilds.cgi
         return true;
   }

   // prevent links in tinderbox.mozilla.org with linkHref to *.gz from open in this function
   if (Tabmix.contentAreaClick.checkAttr(linkNode.toString() , "http://tinderbox.mozilla.org/showlog") ||
      Tabmix.contentAreaClick.checkAttr(linkNode.toString() , "http://tinderbox.mozilla.org/addnote")) return false;

   return Tabmix.contentAreaClick.isUrlForDownload(target.getAttribute("href"));
}
...
function TMP_divertTargetedLink(event, target, linkNode, targetAttr, frames,
  ...
  if (Tabmix.contentAreaClick.checkAttr(linkNode.toString(), "javascript:") || // 2005-11-28 some link in Bloglines start with javascript
      Tabmix.contentAreaClick.checkAttr(linkNode.toString(), "data:"))
    return false;
  ...
  if (target.hasAttribute("onclick")) {
    var onclick = target.getAttribute("onclick");
    if (Tabmix.contentAreaClick.checkAttr(onclick, "window.open") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "NewWindow") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "PopUpWin") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "return "))
          return false;
  }
...
function TMP_openExSiteLink(event, target, linkNode, currentDomain, targetDomain, targetPref) {
  ...
  if (targetDomain && targetDomain != currentDomain ||
     Tabmix.contentAreaClick.checkAttr(target.getAttribute("onmousedown"), "return rwt"))
    return true;
...
function TMP_openTabfromLink(target) {
    ...
    if (Tabmix.contentAreaClick.checkAttr(href, "javascript:") ||
        Tabmix.contentAreaClick.checkAttr(href, "data:") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "window.open") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "openit") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "NewWindow") ||
        Tabmix.contentAreaClick.checkAttr(onclick, "PopUpWin") ||
    ...
    onObserve : function(aSubject, aThis) {
        ...
        try {
          ...
          if (Tabmix.isVersion(40)) {
            ...
            Tabmix.delayedStartup = function() {};
