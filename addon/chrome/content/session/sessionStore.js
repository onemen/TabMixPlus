...
var TMP_ClosedTabs = {
  ...
  removeAllClosedTabs() {
    if (Tabmix.isVersion(1170)) {
      ...
      for (const {closedId, sourceClosedId, sourceWindowId, state} of closedTabsData) {
        if (Tabmix.isVersion(1350) && state.groupId) {
          if (!closedGroupIds.has(state.groupId)) {
            closedGroupIds.add(state.groupId);
            TabmixSvc.ss.forgetClosedTabGroup({sourceClosedId, sourceWindowId}, state.groupId);
          }
        } else {
