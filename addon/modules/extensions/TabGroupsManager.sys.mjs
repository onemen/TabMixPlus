...
export const TMP_TabGroupsManager = {
  init: function TMP_TGM_init(aWindow) {
    ...
    */
    sessionManager._moveTabsToGroupByTGM = function(window, tabs) {
      let sessionStore = window.SessionStore;
    ...
    this.changeCode(sessionManager, "TabmixSessionManager.loadOneWindow")._replace(
      ...
      '  if (jsonText) {' +
      '    if ("__SSi" in window)' +
      '      SessionStore.setWindowValue(window, "TabGroupsManagerAllGroupsData", decodeURI(jsonText));' +
  ...
  _saveAllGroupsData(jsonText, windowNode) {
    if (!this.enableBackup && !windowNode)
      return;
    try {
      let value = jsonText || window.SessionStore.getWindowValue(window, "TabGroupsManagerAllGroupsData");
      if (!windowNode)
